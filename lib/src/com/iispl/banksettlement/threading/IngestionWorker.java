 ...
 *
 * RESPONSIBILITY:
 *   1. Pick the correct adapter from AdapterRegistry by SourceType.
 *   2. Call adapter.adapt(rawPayload) → IncomingTransaction.
 *   3. Check for duplicates (existsBySourceRef).
 *   4. Save to DB with status RECEIVED.
 *   5. Update status to QUEUED.
 *   6. Put IncomingTransaction onto the BlockingQueue.
 *
 * CHANGE LOG (v2):
 *   - AccountDao and CustomerDao dependencies REMOVED.
 *     Account/customer validation has moved to the settlement phase.
 *     The settlement engine reads debitAccount and creditAccount from
 *     normalizedPayload and validates them when building Transaction objects.
 *
 *   - validateAccountsAndCustomers() method REMOVED entirely.
 *
 *   - All transactions now follow a single path: RECEIVED → QUEUED
 *     (unless duplicate, in which case they are silently skipped).
 *     There is no FAILED path in the ingestion phase anymore.
 *
 *   - The log line no longer prints debit/credit account numbers or
 *     requiresAccountValidation (all removed from IncomingTransaction).
 *     Account info is now only inside normalizedPayload.
 */
public class IngestionWorker implements Runnable {
    ...
    public void run() {
        ...
        System.out.println("[IngestionWorker] Thread started for source: " + sourceType
                + " | Thread: " + Thread.currentThread().getName());

        try {
            // STEP 1: adapt raw payload → IncomingTransaction
            TransactionAdapter adapter = adapterRegistry.getAdapter(sourceType);
            IncomingTransaction txn    = adapter.adapt(rawPayload);

            System.out.println("[IngestionWorker] Adapted txn: " + txn.getSourceRef()
            ...
            // STEP 3: save to DB with RECEIVED status
            ...
            // STEP 5: hand off to BlockingQueue for settlement processor
            blockingQueue.put(txn);
            System.out.println("[IngestionWorker] QUEUED txn: " + txn.getSourceRef()
            ...
            // Comment lines and malformed payloads throw this — silently skip
            System.out.println("[IngestionWorker] Skipping line from " + sourceType
                    + ": " + e.getMessage());

        } catch (RuntimeException e) {
            String msg = e.getMessage() != null ? e.getMessage() : e.getClass().getName();
            System.out.println("[IngestionWorker] DB/Runtime ERROR for source [" + sourceType + "]: " + msg);
