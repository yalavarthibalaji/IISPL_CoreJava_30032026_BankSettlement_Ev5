 ...
 *
 * ADDED COLUMNS (run phase3_schema_changes.sql first):
...
public class CreditTransactionDaoImpl implements CreditTransactionDao {
    ...
    public void save(CreditTransaction txn) {
        ...
        try {
            ...
            // incoming_txn_id — use 0 as placeholder if null
    ...
    public CreditTransaction findById(Long txnId) {
        ...
        try {
            conn = ConnectionPool.getConnection();
            ps   = conn.prepareStatement("SELECT * FROM credit_transaction WHERE txn_id = ?");
            ps.setLong(1, txnId);
            rs = ps.executeQuery();
    ...
    public List<CreditTransaction> findByIncomingTxnId(Long incomingTxnId) {
        ...
        try {
            ...
            ps   = conn.prepareStatement(
                    "SELECT * FROM credit_transaction WHERE incoming_txn_id = ?");
            ps.setLong(1, incomingTxnId);
            rs = ps.executeQuery();
            while (rs.next()) list.add(mapRow(rs));
            return list;
        } catch (SQLException e) {
            throw new RuntimeException("CreditTransactionDaoImpl.findByIncomingTxnId() failed: "
                    + e.getMessage(), e);
        } finally {
            closeResources(rs, ps, conn);
        }
    ...
    public void updateStatus(Long txnId, String status) {
        ...
        try {
            conn = ConnectionPool.getConnection();
            ps   = conn.prepareStatement(
                    "UPDATE credit_transaction SET status = ?, updated_at = ? WHERE txn_id = ?");
            ps.setString(1,    status);
            ps.setTimestamp(2, Timestamp.valueOf(LocalDateTime.now()));
            ps.setLong(3,      txnId);
            int rows = ps.executeUpdate();
            if (rows == 0) throw new RuntimeException(
                    "CreditTransactionDaoImpl.updateStatus() — no row for txn_id: " + txnId);
        } catch (SQLException e) {
            throw new RuntimeException("CreditTransactionDaoImpl.updateStatus() failed: "
                    + e.getMessage(), e);
        } finally {
            closeResources(null, ps, conn);
        }
