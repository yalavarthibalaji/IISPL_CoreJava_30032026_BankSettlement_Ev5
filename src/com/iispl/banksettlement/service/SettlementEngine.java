 ...
 * ─────────────────────────────────────────────────────────────────────────
 *
 *  PHASE 1 — PRODUCER (runs in main thread):
 *    Reads ALL INITIATED transactions from 4 DB tables:
 *      credit_transaction    → identifies channel from reference_number prefix
 *      debit_transaction     → identifies channel from reference_number prefix
 ...
 *    Puts all SettlementItems onto a LinkedBlockingQueue.
 *    Puts one SHUTDOWN sentinel at the end.
 *
 *  PHASE 2 — CONSUMER (runs in a separate Thread):
 *    Drains the BlockingQueue item by item.
 *    Groups items into 5 channel buckets (Maps):
 *      cbsItems     → CBS channel  (CREDIT + DEBIT)
 *      rtgsItems    → RTGS channel (INTERBANK only)
 *      neftItems    → NEFT channel (CREDIT + DEBIT)
 *      upiItems     → UPI channel  (CREDIT only)
 *      ftItems      → FT channel   (CREDIT + DEBIT + REVERSAL)
 *
 *  PHASE 3 — BATCH CREATION + SETTLEMENT (inside consumer, after queue is drained):
 *    For each non-empty channel bucket:
 *      a) CREATE a SettlementBatch (INSERT into settlement_batch, status=RUNNING)
 *      b) SETTLE each transaction using the channel's specific rule:
 *           CBS    → direct balance update per transaction
 *           RTGS   → gross settlement per transaction (validate balance first)
 *           NEFT   → NET settlement (sum all debits/credits, apply net once)
 *           UPI    → record as settled (VPA-based, no local balance update)
 *           FT     → best effort (never stop on failure)
 *      c) For each transaction: INSERT into settlement_record
 *      d) UPDATE transaction status to SETTLED or FAILED in its own table
 *      e) FINALIZE batch: update settlement_batch with final status + totals
 *
 * ─────────────────────────────────────────────────────────────────────────
 * CHANNEL DETECTION (from reference_number prefix):
 * ─────────────────────────────────────────────────────────────────────────
 *   CBS-xxxx   → CBS
 *   RTGS-xxxx  → RTGS  (also all interbank_transaction rows)
 *   NEFT-xxxx  → NEFT
 *   UPI-xxxx   → UPI
 *   FT-xxxx    → FT    (also all reversal_transaction rows)
 *
 * ─────────────────────────────────────────────────────────────────────────
 * NETTING (NEFT channel):
 * ─────────────────────────────────────────────────────────────────────────
 *   NEFT settles in net batches — not gross.
 *   Net = SUM(all NEFT credits) - SUM(all NEFT debits)
 *   If net > 0 → net credit position (more money came in than went out)
 *   If net < 0 → net debit position  (more money went out than came in)
 *   Balance update is logged but not applied to local accounts
 *   (in real banking, this net position settles via RBI clearing).
 *   All individual NEFT transactions are still marked SETTLED in DB.
 *
 * ─────────────────────────────────────────────────────────────────────────
 * PACKAGE: com.iispl.banksettlement.service
 * ─────────────────────────────────────────────────────────────────────────
...
public class SettlementEngine {
    ...
    private static final String SQL_LOAD_INITIATED_CREDITS =
            "SELECT txn_id, debit_account_id, credit_account_id, credit_account_number, " +
            "amount, currency, txn_date, value_date, status, reference_number, txn_type " +
            "FROM credit_transaction WHERE status = 'INITIATED' AND reference_number LIKE ? " +
            "ORDER BY txn_id ASC";

    private static final String SQL_LOAD_INITIATED_DEBITS =
            "SELECT txn_id, debit_account_id, credit_account_id, debit_account_number, " +
            "amount, currency, txn_date, value_date, status, reference_number, txn_type " +
            "FROM debit_transaction WHERE status = 'INITIATED' AND reference_number LIKE ? " +
            "ORDER BY txn_id ASC";

    private static final String SQL_LOAD_INITIATED_INTERBANK =
            "SELECT txn_id, debit_account_id, credit_account_id, correspondent_bank_code, " +
            "amount, currency, txn_date, value_date, status, reference_number, txn_type " +
            "FROM interbank_transaction WHERE status = 'INITIATED' " +
            "ORDER BY txn_id ASC";

    private static final String SQL_LOAD_INITIATED_REVERSALS =
            "SELECT txn_id, debit_account_id, credit_account_id, original_txn_ref, " +
            "reversal_reason, amount, currency, txn_date, value_date, status, reference_number " +
            "FROM reversal_transaction WHERE status = 'INITIATED' " +
            "ORDER BY txn_id ASC";

    // SQL for account balance operations
    private static final String SQL_CREDIT_BALANCE =
            "UPDATE account SET balance = balance + ?, updated_at = now() WHERE account_id = ?";

    private static final String SQL_DEBIT_BALANCE =
            "UPDATE account SET balance = balance - ?, updated_at = now() WHERE account_id = ?";
    ...
    private static final String SQL_FIND_INCOMING_TXN_ID =
            "SELECT incoming_txn_id FROM incoming_transaction WHERE source_ref = ?";
     ...
     *   Phase 1 → load all INITIATED transactions → put on BlockingQueue
     *   Phase 2 → consumer thread drains queue → groups by channel
    ...
    public void runSettlement() {
        ...
        System.out.println("\n[SettlementEngine] PHASE 1 DONE — Loaded "
                + totalLoaded + " transaction(s) onto the queue.");
        ...
        Runnable consumer = () -> {
            ...
            System.out.println("\n[SettlementEngine] PHASE 2 — Consumer thread started: "
            ...
            while (true) {
                try {
                    ...
                    switch (item.getChannel()) {
                        case "CBS":  cbsItems.add(item);  break;
                        case "RTGS": rtgsItems.add(item); break;
                        case "NEFT": neftItems.add(item); break;
                        case "UPI":  upiItems.add(item);  break;
                        case "FT":   ftItems.add(item);   break;
                        default:
                            System.out.println("[SettlementEngine] UNKNOWN channel: "
                                    + item.getChannel() + " — skipping item: " + item);
                    }
            ...
            if (!cbsItems.isEmpty())  settleCbsBatch(cbsItems);
            if (!rtgsItems.isEmpty()) settleRtgsBatch(rtgsItems);
            if (!neftItems.isEmpty()) settleNeftBatch(neftItems);
            if (!upiItems.isEmpty())  settleUpiBatch(upiItems);
            if (!ftItems.isEmpty())   settleFtBatch(ftItems);
    ...
    /**
     * Loads all INITIATED transactions from all 4 tables.
     * Wraps each into a SettlementItem with the correct channel.
     * Puts all items onto the queue.
     *
    ...
    private int loadCreditsForChannel(BlockingQueue<SettlementItem> queue,
        ...
        try {
            ...
            throw new RuntimeException(
                "loadCreditsForChannel() failed for prefix: " + prefix + " | " + e.getMessage(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } finally {
    ...
    private int loadDebitsForChannel(BlockingQueue<SettlementItem> queue,
                                      String prefix, String channel) {
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try {
            ...
            throw new RuntimeException(
                "loadDebitsForChannel() failed for prefix: " + prefix + " | " + e.getMessage(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } finally {
            closeResources(rs, ps, conn);
    ...
    private int loadInterbankTransactions(BlockingQueue<SettlementItem> queue) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                queue.put(item);
                count++;
                System.out.println("[SettlementEngine] Queued INTERBANK [RTGS] | ref: "
                        + txn.getReferenceNumber() + " | amount: " + txn.getAmount()
                        + " | correspondent: " + txn.getCorrespondentBankCode());
            }
        } catch (SQLException e) {
            throw new RuntimeException(
                "loadInterbankTransactions() failed: " + e.getMessage(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } finally {
            closeResources(rs, ps, conn);
    ...
    private int loadReversalTransactions(BlockingQueue<SettlementItem> queue) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                queue.put(item);
                count++;
                System.out.println("[SettlementEngine] Queued REVERSAL [FT] | ref: "
                        + txn.getReferenceNumber() + " | amount: " + txn.getAmount()
                        + " | originalRef: " + txn.getOriginalTxnRef());
            }
        } catch (SQLException e) {
            throw new RuntimeException(
                "loadReversalTransactions() failed: " + e.getMessage(), e);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        } finally {
            closeResources(rs, ps, conn);
            ...
            try {
                if ("CREDIT".equals(item.getTxnType())) {
                    ...
                    SettlementRecord rec = new SettlementRecord(
                        batchId, item.getIncomingTxnId(), txn.getAmount(),
                        SettlementStatus.SETTLED, null);
                    batchDao.saveRecord(rec);
                    batch.addRecord(rec);
                    settled++;
                    System.out.println("[CBS] CREDIT settled | ref: " + txn.getReferenceNumber()
                    ...
                    validateAmount(txn.getAmount(), txn.getReferenceNumber());
                    checkSufficientBalance(txn.getDebitAccountId(), txn.getAmount(),
                    ...
                    batch.addRecord(rec);
                    settled++;
                    System.out.println("[CBS] DEBIT settled | ref: " + txn.getReferenceNumber()
                            + " | amount: " + txn.getAmount());
                }
    ...
    private void settleRtgsBatch(List<SettlementItem> items) {
        ...
        System.out.println("\n--- [RTGS] Settling " + items.size()
                + " interbank transaction(s) GROSS ---");
        ...
        for (SettlementItem item : items) {
            ...
            try {
                ...
                batch.addRecord(rec);
                settled++;
                System.out.println("[RTGS] GROSS settled | ref: " + txn.getReferenceNumber()
                        + " | amount: " + txn.getAmount()
                        + " | correspondent: " + txn.getCorrespondentBankCode());
    ...
    private void settleNeftBatch(List<SettlementItem> items) {
        ...
        System.out.println("\n--- [NEFT] Settling " + items.size()
                + " transaction(s) using NET settlement ---");
        ...
        System.out.println("[NEFT] Net Amount    : " + netAmount
                + (netAmount.compareTo(BigDecimal.ZERO) >= 0
                   ? "  ← NET CREDIT POSITION (more money received)"
        ...
        for (SettlementItem item : items) {
            try {
                if ("CREDIT".equals(item.getTxnType())) {
                    ...
                    batch.addRecord(rec);
                    settled++;
                    System.out.println("[NEFT] CREDIT recorded (net) | ref: "
                            + txn.getReferenceNumber() + " | amount: " + txn.getAmount());
                    ...
                    batch.addRecord(rec);
                    settled++;
                    System.out.println("[NEFT] DEBIT recorded (net) | ref: "
                            + txn.getReferenceNumber() + " | amount: " + txn.getAmount());
                }
            } catch (Exception e) {
                failed++;
        ...
        System.out.println("[NEFT] Net amount: " + netAmount
                + (netAmount.compareTo(BigDecimal.ZERO) >= 0
                   ? " → Bank receives this net amount via RBI clearing"
    ...
    private void settleUpiBatch(List<SettlementItem> items) {
        ...
        System.out.println("\n--- [UPI] Settling " + items.size()
                + " VPA transaction(s) (no local balance update) ---");
        ...
        for (SettlementItem item : items) {
            try {
                ...
                batch.addRecord(rec);
                settled++;
                System.out.println("[UPI] Settled (VPA, no balance update) | ref: "
                        + txn.getReferenceNumber() + " | amount: " + txn.getAmount());
    ...
    private void settleFtBatch(List<SettlementItem> items) {
        ...
        System.out.println("\n--- [FT] Settling " + items.size()
                + " FinTech transaction(s) (best effort) ---");
        ...
        for (SettlementItem item : items) {
            ...
            try {
                if ("CREDIT".equals(item.getTxnType())) {
                    ...
                    batch.addRecord(rec);
                    settled++;
                    System.out.println("[FT] REVERSAL settled | ref: " + txn.getReferenceNumber()
                            + " | originalRef: " + txn.getOriginalTxnRef());
                }
     ...
     * Throws RuntimeException if invalid — caught by channel settlers.
     */
    ...
    /**
     * Checks that an account has enough balance for a debit.
     * Throws RuntimeException if insufficient — caught by channel settlers.
     */
    private void checkSufficientBalance(Long accountId, BigDecimal required, String ref) {
        if (accountId == null || accountId == 0L) return; // skip for placeholder IDs
        BigDecimal balance = getBalance(accountId);
        if (balance.compareTo(required) < 0) {
            throw new RuntimeException(
                "Insufficient funds | accountId: " + accountId
                + " | balance: " + balance
                + " | required: " + required
                + " | ref: " + ref);
        }
    ...
    /**
     * Handles a failed settlement item:
     *   - Logs the error
     *   - Creates a FAILED SettlementRecord and saves it
     *   - Adds it to the batch
     *   - Updates the transaction status to FAILED in its own table
     */
    private void handleFailure(SettlementItem item, String batchId,
                                SettlementBatch batch, String reason) {
        ...
        System.out.println("[SettlementEngine] FAILED | channel: " + item.getChannel()
                + " | type: " + item.getTxnType()
                + " | ref: " + ref
                + " | reason: " + reason);
    ...
    private void finalizeBatch(SettlementBatch batch, int settled, int failed, String channel) {
        ...
        System.out.println("[" + channel + "] Batch finalized | batchId: " + batch.getBatchId()
    ...
    /** Adds amount to an account's balance (CREDIT). Skips if accountId is null or 0. */
    private void creditBalance(Long accountId, BigDecimal amount) {
        if (accountId == null || accountId == 0L) return;
        Connection conn = null;
        PreparedStatement ps = null;
        try {
            conn = ConnectionPool.getConnection();
            ...
            throw new RuntimeException(
                "creditBalance() failed for accountId: " + accountId + " | " + e.getMessage(), e);
        } finally {
            closeResources(null, ps, conn);
        }
    }

    /** Subtracts amount from an account's balance (DEBIT). Skips if accountId is null or 0. */
    private void debitBalance(Long accountId, BigDecimal amount) {
        ...
        try {
            ...
            throw new RuntimeException(
                "debitBalance() failed for accountId: " + accountId + " | " + e.getMessage(), e);
        } finally {
            closeResources(null, ps, conn);
        }
    }
    ...
    private BigDecimal getBalance(Long accountId) {
        ...
        try {
            ...
            throw new RuntimeException(
                "getBalance() failed for accountId: " + accountId + " | " + e.getMessage(), e);
        } finally {
            closeResources(rs, ps, conn);
        }
    }
    ...
    /**
     * Looks up the incoming_txn_id from incoming_transaction table
     * using the source_ref (which equals reference_number in transaction tables).
     * Returns 0L if not found.
     */
    private Long findIncomingTxnId(String sourceRef) {
        if (sourceRef == null) return 0L;
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) return rs.getLong("incoming_txn_id");
            return 0L;
        } catch (SQLException e) {
            System.out.println("[SettlementEngine] Could not find incoming_txn_id for ref: "
                    + sourceRef + " | " + e.getMessage());
            return 0L;
        } finally {
            closeResources(rs, ps, conn);
        }
