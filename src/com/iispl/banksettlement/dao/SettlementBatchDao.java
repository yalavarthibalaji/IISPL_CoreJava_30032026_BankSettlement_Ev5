...
/**
 * SettlementBatchDao — Interface for all DB operations on settlement_batch and
 * settlement_record tables.
 *
 * WHY TWO TABLES IN ONE DAO? SettlementBatch HAS-A List<SettlementRecord> —
 * COMPOSITION. A SettlementRecord cannot exist without a batch. So both tables
 * are managed together from one DAO.
 *
...
public interface SettlementBatchDao {
	...
	/**
	 * Inserts a new row into settlement_batch. Call this BEFORE processing any
	 * transactions — status will be RUNNING.
	 *
	 ...
	 * Call this AFTER all transactions in the batch are processed. Final status
	 * will be: COMPLETED, PARTIAL, or FAILED.
	 *
	...
	/**
	 * Inserts one SettlementRecord into settlement_record table. Call this once per
	 * transaction after it is settled or failed. DB-generated record_id is set back
	 * on the object after save.
	 *
