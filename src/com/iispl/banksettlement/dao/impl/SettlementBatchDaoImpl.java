 ...
 *
 * Handles two DB tables: 1. settlement_batch — one row per channel batch 2.
 * settlement_record — one row per transaction settled inside a batch
 *
 * HOW THE ENGINE USES THIS: Step 1 → saveBatch() → INSERT into settlement_batch
 * (status=RUNNING) Step 2 → saveRecord() → INSERT into settlement_record (one
 * per txn) Step 3 → updateBatch() → UPDATE settlement_batch totals + final
 * status
 *
 * DB TABLE COLUMNS:
 *
 * settlement_batch: batch_id, batch_date, batch_status, total_transactions,
 * total_amount, run_by, run_at, created_at, updated_at, created_by, version
 *
 * settlement_record: record_id(serial), batch_id, incoming_txn_id,
...
public class SettlementBatchDaoImpl implements SettlementBatchDao {
	...
	public void saveBatch(SettlementBatch batch) {
		...
		String SQL_INSERT_BATCH = "INSERT INTO settlement_batch "
		...
		try {
			...
			LocalDateTime runAt = (batch.getRunAt() != null) ? batch.getRunAt() : LocalDateTime.now();
	...
	public void updateBatch(SettlementBatch batch) {
		...
		String SQL_UPDATE_BATCH = "UPDATE settlement_batch "
	...
	public SettlementBatch findBatchById(String batchId) {
		...
		String SQL_FIND_BATCH_BY_ID = "SELECT * FROM settlement_batch WHERE batch_id = ?";
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next())
				return mapBatchRow(rs);
			return null;
		} catch (SQLException e) {
			throw new RuntimeException("SettlementBatchDaoImpl.findBatchById() failed: " + e.getMessage(), e);
		} finally {
	...
	public void saveRecord(SettlementRecord record) {
		...
		String SQL_INSERT_RECORD = "INSERT INTO settlement_record "
		...
		try {
			...
			LocalDateTime settledDate = (record.getSettledDate() != null) ? record.getSettledDate()
					: LocalDateTime.now();
			ps.setTimestamp(4, Timestamp.valueOf(settledDate));

			ps.setString(5, record.getSettledStatus().name());
			// failure_reason is null for successful settlements — setString handles null
			// correctly
			...
			if (rows == 0) {
				throw new RuntimeException(
						"SettlementBatchDaoImpl.saveRecord() — no row inserted for batchId: " + record.getBatchId());
			}
	...
	public List<SettlementRecord> findRecordsByBatchId(String batchId) {
		...
		String SQL_FIND_RECORDS_BY_BATCH = "SELECT * FROM settlement_record WHERE batch_id = ? ORDER BY record_id ASC";
		...
		try {
			...
			throw new RuntimeException("SettlementBatchDaoImpl.findRecordsByBatchId() failed: " + e.getMessage(), e);
		} finally {
			closeResources(rs, ps, conn);
		}
	}
	...
	private SettlementBatch mapBatchRow(ResultSet rs) throws SQLException {
		...
		if (runAt != null)
			batch.setRunAt(runAt.toLocalDateTime());
		return batch;
	}
	...
	private SettlementRecord mapRecordRow(ResultSet rs) throws SQLException {
		...
		if (sd != null)
			rec.setSettledDate(sd.toLocalDateTime());
