 ...
 *
 * CHANGE LOG (v2): - currency column REMOVED from SQL_INSERT and
 * mapRowToIncomingTransaction(). The DB schema column "currency" has also been
 * dropped (see schema migration note). Currency information is still preserved
 ...
 *
 * If you want to keep the column for legacy data but stop writing to it: ALTER
 * TABLE incoming_transaction ALTER COLUMN currency DROP NOT NULL; (In this case
...
public class IncomingTransactionDaoImpl implements IncomingTransactionDao {
	...
	public void save(IncomingTransaction txn) {
		...
		String SQL_INSERT = "INSERT INTO incoming_transaction "
		...
		try {
			...
			if (rowsAffected == 0) {
				throw new RuntimeException("IncomingTransactionDaoImpl.save() failed — no rows inserted for sourceRef: "
						+ txn.getSourceRef());
			}
	...
	public IncomingTransaction findById(Long incomingTxnId) {
		...
		String SQL_FIND_BY_ID = "SELECT t.*, s.system_code, s.protocol, s.connection_config, "
				+ "s.is_active, s.contact_email " + "FROM incoming_transaction t "
				+ "JOIN source_system s ON t.source_system_id = s.source_system_id " + "WHERE t.incoming_txn_id = ?";
		...
		try {
			...
			throw new RuntimeException("IncomingTransactionDaoImpl.findById() failed: " + e.getMessage(), e);
		} finally {
	...
	public IncomingTransaction findBySourceRef(String sourceRef) {
		...
		String SQL_FIND_BY_SOURCE_REF = "SELECT t.*, s.system_code, s.protocol, s.connection_config, "
				+ "s.is_active, s.contact_email " + "FROM incoming_transaction t "
				+ "JOIN source_system s ON t.source_system_id = s.source_system_id " + "WHERE t.source_ref = ?";
		...
		try {
			...
			throw new RuntimeException("IncomingTransactionDaoImpl.findBySourceRef() failed: " + e.getMessage(), e);
		} finally {
			closeResources(rs, ps, conn);
		}
	}
	...
	public boolean existsBySourceRef(String sourceRef) {
		...
		String SQL_EXISTS_BY_SOURCE_REF = "SELECT COUNT(*) FROM incoming_transaction WHERE source_ref = ?";
		...
		try {
			...
			throw new RuntimeException("IncomingTransactionDaoImpl.existsBySourceRef() failed: " + e.getMessage(), e);
		} finally {
			closeResources(rs, ps, conn);
		}
	}
	...
	public List<IncomingTransaction> findByStatus(ProcessingStatus status) {
		...
		String SQL_FIND_BY_STATUS = "SELECT t.*, s.system_code, s.protocol, s.connection_config, "
				+ "s.is_active, s.contact_email " + "FROM incoming_transaction t "
				+ "JOIN source_system s ON t.source_system_id = s.source_system_id " + "WHERE t.processing_status = ? "
		...
		try {
			...
			throw new RuntimeException("IncomingTransactionDaoImpl.findByStatus() failed: " + e.getMessage(), e);
		} finally {
			closeResources(rs, ps, conn);
		}
	}
	...
	public void updateStatus(Long incomingTxnId, ProcessingStatus status) {
		...
		String SQL_UPDATE_STATUS = "UPDATE incoming_transaction " + "SET processing_status = ?, updated_at = ? "
		...
		try {
			...
			if (rowsAffected == 0) {
				throw new RuntimeException(
						"IncomingTransactionDaoImpl.updateStatus() — no row found for id: " + incomingTxnId);
			}

		} catch (SQLException e) {
			throw new RuntimeException("IncomingTransactionDaoImpl.updateStatus() failed: " + e.getMessage(), e);
		} finally {
			closeResources(ps, conn);
		}
	}
