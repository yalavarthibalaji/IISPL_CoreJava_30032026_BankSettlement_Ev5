...
/**
 * CreditTransactionDaoImpl — JDBC implementation for credit_transaction table.
 *
 * ACTUAL TABLE COLUMNS (from Supabase schema): txn_id, debit_account_id,
 * credit_account_id, credit_account_number, amount, currency, txn_date,
 * value_date, status, reference_number, txn_type, created_at, updated_at,
 * created_by, version
 *
 * NOTE: There is NO incoming_txn_id column in this table. The link back to
 * incoming_transaction is via reference_number which stores the sourceRef from
 * IncomingTransaction.
 */
public class CreditTransactionDaoImpl implements CreditTransactionDao {
	...
	public void save(CreditTransaction txn) {
		...
		String SQL_INSERT = "INSERT INTO credit_transaction "
				+ "(debit_account_id, credit_account_id, credit_account_number, "
		...
		try {
			...
			ps.setString(3, txn.getCreditAccountNumber());
			...
			if (rowsAffected == 0) {
				throw new RuntimeException("CreditTransactionDaoImpl.save() failed — no rows inserted for ref: "
			...
			System.out.println("[CreditTransactionDaoImpl] Saved | txn_id: " + txn.getTxnId() + " | ref: "
					+ txn.getReferenceNumber() + " | amount: " + txn.getAmount());

		} catch (SQLException e) {
			throw new RuntimeException("CreditTransactionDaoImpl.save() failed: " + e.getMessage(), e);
		} finally {
	...
	@Override
	public CreditTransaction findById(Long txnId) {

		String SQL_FIND_BY_ID = "SELECT * FROM credit_transaction WHERE txn_id = ?";
		...
		try {
			...
			throw new RuntimeException("CreditTransactionDaoImpl.findById() failed: " + e.getMessage(), e);
		} finally {
	...
	@Override
	public List<CreditTransaction> findByIncomingTxnId(Long incomingTxnId) {
	...
	public void updateStatus(Long txnId, String status) {
		...
		String SQL_UPDATE_STATUS = "UPDATE credit_transaction SET status = ?, updated_at = ? WHERE txn_id = ?";
		...
		try {
			...
			int rowsAffected = ps.executeUpdate();
			if (rowsAffected == 0) {
				throw new RuntimeException("CreditTransactionDaoImpl.updateStatus() — no row for txn_id: " + txnId);
			}

		} catch (SQLException e) {
			throw new RuntimeException("CreditTransactionDaoImpl.updateStatus() failed: " + e.getMessage(), e);
		} finally {
			closeResources(ps, conn);
		}
	}
