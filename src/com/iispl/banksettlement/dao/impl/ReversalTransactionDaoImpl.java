...
/**
 * ReversalTransactionDaoImpl — JDBC implementation for reversal_transaction table.
 *
 * ACTUAL TABLE COLUMNS (from Supabase schema):
 *   txn_id, debit_account_id, credit_account_id, original_txn_ref,
 *   reversal_reason, amount, currency, txn_date, value_date, status,
 *   reference_number, txn_type, created_at, updated_at, created_by, version
 *
...
public class ReversalTransactionDaoImpl implements ReversalTransactionDao {
    ...
    private static final String SQL_FIND_BY_ID =
            "SELECT * FROM reversal_transaction WHERE txn_id = ?";

    private static final String SQL_FIND_BY_ORIGINAL_TXN_REF =
    ...
    public void save(ReversalTransaction txn) {
        ...
        try {
            ...
            System.out.println("[ReversalTransactionDaoImpl] Saved | txn_id: " + txn.getTxnId()
                    ...
                    + " | reason: "      + txn.getReversalReason());

        } catch (SQLException e) {
            throw new RuntimeException(
                "ReversalTransactionDaoImpl.save() failed: " + e.getMessage(), e);
        } finally {
    ...
    public ReversalTransaction findById(Long txnId) {
        ...
        try {
            ...
            throw new RuntimeException(
                "ReversalTransactionDaoImpl.findById() failed: " + e.getMessage(), e);
        } finally {
    ...
    public List<ReversalTransaction> findByOriginalTxnRef(String originalTxnRef) {
        ...
        try {
            ...
            throw new RuntimeException(
                "ReversalTransactionDaoImpl.findByOriginalTxnRef() failed: " + e.getMessage(), e);
        } finally {
            closeResources(rs, ps, conn);
        }
    }
    ...
    public void updateStatus(Long txnId, String status) {
        ...
        try {
            ...
            throw new RuntimeException(
                "ReversalTransactionDaoImpl.updateStatus() failed: " + e.getMessage(), e);
        } finally {
            closeResources(ps, conn);
        }
    }
