 ...
 * credit_account_id, debit_account_number, amount, currency, txn_date,
...
public class DebitTransactionDaoImpl implements DebitTransactionDao {
	...
	public void save(DebitTransaction txn) {
		...
		try {
			...
			if (rowsAffected == 0) {
				throw new RuntimeException("DebitTransactionDaoImpl.save() failed — no rows inserted for ref: "
			...
			System.out.println("[DebitTransactionDaoImpl] Saved | txn_id: " + txn.getTxnId() + " | ref: "
	...
	public DebitTransaction findById(Long txnId) {
		...
		String SQL_FIND_BY_ID = "SELECT * FROM debit_transaction WHERE txn_id = ?";
	...
	public void updateStatus(Long txnId, String status) {
		...
		String SQL_UPDATE_STATUS = "UPDATE debit_transaction SET status = ?, updated_at = ? WHERE txn_id = ?";
		...
		try {
			...
			int rowsAffected = ps.executeUpdate();
			if (rowsAffected == 0) {
				throw new RuntimeException("DebitTransactionDaoImpl.updateStatus() — no row for txn_id: " + txnId);
			}

		} catch (SQLException e) {
			throw new RuntimeException("DebitTransactionDaoImpl.updateStatus() failed: " + e.getMessage(), e);
		} finally {
			closeResources(ps, conn);
		}
	}
