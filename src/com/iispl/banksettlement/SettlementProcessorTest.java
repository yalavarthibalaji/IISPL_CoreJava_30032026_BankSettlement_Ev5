 ...
 * ─────────────────────────────────────────────────────────────────────────
 *
 *  MODE A — Run from existing QUEUED records in DB (no new file ingestion):
 *    1. Calls IncomingTransactionDao.findByStatus(QUEUED) to load all
 *       already-queued transactions from the DB.
 *    2. Puts them onto a fresh BlockingQueue.
 *    3. Starts TransactionDispatcher in a background thread.
 *    4. Dispatcher reads from queue, parses normalizedPayload,
 *       builds CreditTransaction / DebitTransaction / ReversalTransaction /
 *       InterBankTransaction, saves to the correct DB table,
 *       marks incoming_transaction as PROCESSED.
 *
 *  MODE B — Ingest NEW test files first, then dispatch (full pipeline test):
 *    1. Runs IngestionPipeline to read updated test files
 *       (CBS CSV, RTGS XML, NEFT TXT, UPI JSON, Fintech JSON).
 *    2. Each adapter converts raw payload → IncomingTransaction → DB → BlockingQueue.
 *    3. After ingestion is done, same dispatcher loop processes the queue.
 *
 * HOW TO CHOOSE A MODE:
 *   Set the constant RUN_MODE below to MODE_A or MODE_B.
 *
 * PREREQUISITES:
 *   1. Supabase project is active and db.properties is configured.
 *   2. Phase 1 schema already run (incoming_transaction, account, etc. exist).
 *   3. Phase 2 schema already run (credit_transaction, debit_transaction,
 *      reversal_transaction, interbank_transaction tables exist).
 *   4. Account INSERT statements from phase2_schema.sql have been run
 *      (so debit/credit account FKs can be resolved).
 *   5. For MODE B: updated test files are in testfiles/ folder.
 *
 * EXPECTED RESULTS AFTER RUNNING:
 *   - incoming_transaction: processing_status changes from QUEUED → PROCESSED
 *     (or FAILED if account not found).
...
public class SettlementProcessorTest {
    ...
    private static void runModeA() {
        ...
        if (queuedTxns.isEmpty()) {
            ...
            System.out.println("[SettlementProcessorTest] >>> Run FileIngestionTest first to populate incoming_transaction.");
        ...
        for (IncomingTransaction txn : queuedTxns) {
            try {
                ...
                System.out.println("[SettlementProcessorTest] Queued: "
                        + txn.getSourceRef()
                        + " | type: " + txn.getTxnType()
    ...
    private static void runModeB() {
        ...
        ingestFile(pipeline, SourceType.FINTECH,FILE_BASE_PATH + "fintech_transactions.json",new JsonFileReader("FINTECH_JSON"));

        System.out.println("\n[SettlementProcessorTest] Ingestion submitted. Waiting for workers...");

        // STEP 2: Wait for all ingestion workers to finish, but do NOT close ConnectionPool yet.
        ...
        System.out.println("[SettlementProcessorTest] Queue size after ingestion: "
                + blockingQueue.size());
    ...
    private static void runDispatcher(BlockingQueue<IncomingTransaction> blockingQueue) {
        ...
        try {
            // Wait for the dispatcher thread to finish (it stops when it sees SHUTDOWN signal)
    ...
    private static void ingestFile(IngestionPipeline pipeline,
        ...
        try {
            ...
            System.out.println("[SettlementProcessorTest] ERROR reading file [" + filePath + "]: "
                    + e.getMessage());
            return;
        }
        ...
        System.out.println("[SettlementProcessorTest] Submitting " + payloads.size()
                + " record(s) from [" + reader.getSourceFormat() + "]...");
