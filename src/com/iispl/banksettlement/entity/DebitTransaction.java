...
/**
 * DebitTransaction — Represents a transaction where money goes OUT of an account.
 *
 * Examples:
 *   - A customer initiates an outgoing NEFT payment
 *   - An outgoing RTGS transfer to another bank
 *   - A bill payment being processed
 *
 * IS-A Transaction (extends Transaction)
 * IS-A BaseEntity  (inherits through Transaction)
 *
 * Extra field in this subclass:
 *   - debitAccountNumber : The actual bank account number from which money is taken
 *                          (in addition to the debitAccountId FK from Transaction)
 *
 * FLOW:
 *   IncomingTransaction (DEBIT type) → gets converted to DebitTransaction
...
public class DebitTransaction extends Transaction {
    ...
    /**
     * Default constructor.
     * txnType is automatically set to DEBIT.
     */
     ...
     */
    public DebitTransaction(Long debitAccountId, String debitAccountNumber,
                            Long creditAccountId,
                            BigDecimal amount, String currency,
                            LocalDate valueDate, String referenceNumber) {
        super(debitAccountId, creditAccountId, amount, currency,
              valueDate, referenceNumber, TransactionType.DEBIT);
