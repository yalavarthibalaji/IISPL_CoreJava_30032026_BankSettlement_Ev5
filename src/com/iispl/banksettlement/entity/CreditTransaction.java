...
/**
 * CreditTransaction — Represents a transaction where money comes INTO an account.
 *
 * Examples:
 *   - A customer receives a salary payment
 *   - An incoming NEFT transfer from another bank
 *   - An incoming SWIFT wire from a foreign bank
 *
 * IS-A Transaction (extends Transaction)
 * IS-A BaseEntity  (inherits through Transaction)
 *
 * Extra field in this subclass:
 *   - creditAccountNumber : The actual bank account number receiving the money
 *                           (in addition to the creditAccountId FK from Transaction)
 *
 * FLOW:
 *   IncomingTransaction (CREDIT type) → gets converted to CreditTransaction
...
public class CreditTransaction extends Transaction {
    ...
    /**
     * Default constructor.
     * txnType is automatically set to CREDIT.
     */
     ...
     */
    public CreditTransaction(Long debitAccountId, Long creditAccountId,
                             String creditAccountNumber,
                             BigDecimal amount, String currency,
                             LocalDate valueDate, String referenceNumber) {
        super(debitAccountId, creditAccountId, amount, currency,
              valueDate, referenceNumber, TransactionType.CREDIT);
