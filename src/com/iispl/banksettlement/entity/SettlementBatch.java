 ...
 *   Instead of settling each transaction one by one (which is slow and expensive),
 *   the settlement engine groups many transactions into a batch and settles them
 *   together. This is how real banking systems like NEFT work — they run in
 *   cycles and settle all transactions in that cycle as one batch.
 *
 * IS-A BaseEntity (extends BaseEntity)
 * Inherits: id, createdAt, updatedAt, createdBy, version
 *
 * HAS-A List<SettlementRecord> — COMPOSITION relationship
 *   A SettlementBatch contains a list of SettlementRecord objects.
 *   Each SettlementRecord represents one individual transaction that was
 *   settled as part of this batch.
 *   If the batch is deleted, all its records are also deleted (composition).
 *
...
public class SettlementBatch extends BaseEntity {
     ...
     * Records list is initialized empty.
     * Total amounts start at ZERO.
     */
    ...
    /**
     * Adds a SettlementRecord to this batch and updates the totals.
     * Call this each time a transaction is settled and added to the batch.
     *
