 ...
 *
 * Examples: - An RTGS transfer from our bank to another bank - A SWIFT
 * cross-border wire transfer - An interbank settlement instruction through RBI
 *
 * IS-A Transaction (extends Transaction) IS-A BaseEntity (inherits through
 * Transaction)
 *
 * Extra field in this subclass: - correspondentBankCode : The IFSC or BIC/SWIFT
 * code of the other bank involved in this interbank transaction.
 * "correspondent" means the partner/counterparty bank.
 *
 * FLOW: Incoming RTGS/SWIFT message → adapt → InterBankTransaction →
 * NettingEngine calculates bilateral positions → SettlementEngine settles via
 * RTGS/SWIFT rail
 */
public class InterBankTransaction extends Transaction {
	...
	/**
	 * Default constructor. txnType is automatically set to INTRABANK. (INTRABANK in
	 * enums covers interbank transfers — between banks)
	 */
	 ...
	 * @param creditAccountId       ID of the account being credited (could be at
	 *                              other bank)
	 ...
	 */
	public InterBankTransaction(Long debitAccountId, Long creditAccountId, String correspondentBankCode,
			BigDecimal amount, String currency, LocalDate valueDate, String referenceNumber) {
		super(debitAccountId, creditAccountId, amount, currency, valueDate, referenceNumber, TransactionType.INTRABANK);
	...
	public String toString() {
		return "InterBankTransaction{" + "txnId=" + getTxnId() + ", correspondentBankCode='" + correspondentBankCode
				+ '\'' + ", debitAccountId=" + getDebitAccountId() + ", creditAccountId=" + getCreditAccountId()
				+ ", amount=" + getAmount() + ", currency='" + getCurrency() + '\'' + ", valueDate=" + getValueDate()
				+ ", status=" + getStatus() + ", referenceNumber='" + getReferenceNumber() + '\'' + '}';
	}
}
