 ...
 * ─────────────────────────────────────────────────────────────────────────────
 *  CBS_TXN_ID    [0]        C001
 *  ACCT_DR       [1]        ACC-HDFC-001       ← debit account
 *  ACCT_CR       [2]        ACC-HDFC-002       ← credit account
 *  AMT           [3]        25000.00
 *  CCY           [4]        INR
 *  TXN_DT        [5]        20260402           ← format: yyyyMMdd
 *  TXN_TYPE      [6]        CREDIT / DEBIT
 *  NARRATION     [7]        Salary credit March 2026
 *  BRANCH_CODE   [8]        BLR001
 *  MAKER_ID      [9]        EMP4521
 *  FROM_BANK     [10]       HDFC Bank          ← NEW (v3) — bank owning ACCT_DR
 *  TO_BANK       [11]       SBI Bank           ← NEW (v3) — bank owning ACCT_CR
 * ─────────────────────────────────────────────────────────────────────────────
 *
 * EXAMPLE LINE:
 *   C001|ACC-HDFC-001|ACC-HDFC-002|25000.00|INR|20260402|CREDIT|Salary credit March 2026|BLR001|EMP4521|HDFC Bank|SBI Bank
 *
 * CHANGE LOG (v3 — fromBank / toBank):
 *   - FROM_BANK [10] and TO_BANK [11] added as new pipe-delimited fields.
 *   - Both are parsed and stored as:
 *       (a) txn.fromBank / txn.toBank fields on IncomingTransaction.
 *       (b) "fromBank" / "toBank" keys inside normalizedPayload JSON.
 *   - CsvFileReader updated to read 12 columns instead of 10.
 *   - cbs_transactions.csv updated with FROM_BANK and TO_BANK columns.
 */
public class CbsAdapter implements TransactionAdapter {
    ...
    public IncomingTransaction adapt(String rawPayload) {
        ...
        if (parts.length < 12) {
            throw new IllegalArgumentException(
                "CbsAdapter: Invalid CBS payload. Expected 12 pipe-separated fields: " +
                "CBS_TXN_ID|ACCT_DR|ACCT_CR|AMT|CCY|TXN_DT|TXN_TYPE|NARRATION" +
                "|BRANCH_CODE|MAKER_ID|FROM_BANK|TO_BANK" +
                ". Got " + parts.length + " fields: " + rawPayload
            );
        }
