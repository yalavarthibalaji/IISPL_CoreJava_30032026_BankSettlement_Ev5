 ...
 *  PURPOSE_CODE   123-126     4        OTHR / RVRSL
 *  BATCH_NO       127-132     6        B00012
 *  FROM_BANK      133-152     20       ICICI Bank
 *  TO_BANK        153-172     20       SBI Bank
 * ──────────────────────────────────────────────────────────────────────────
 *
 * RECORD_TYPE values:
 *   CR  → CREDIT     (money coming into BENE_ACCT)
 *   DR  → DEBIT      (money going out of SENDER_ACCT)
 *   RV  → REVERSAL   (undo of a previous NEFT transaction)
 *
 * For REVERSAL records (RV):
 *   - SENDER_ACCT holds the original creditAccount (money flows back)
 *   - BENE_ACCT   holds the original debitAccount  (money returns here)
 *   - NEFT_REF    is a NEW unique reference for this reversal
 *   - PURPOSE_CODE is RVRSL
 *   - The originalTxnRef is derived: swap the date in NEFT_REF to find original
 *     (Settlement engine uses this to locate the original transaction)
 *
 * CHANGE LOG (v4 — REVERSAL support added):
 *   - RECORD_TYPE switch now handles "RV" → TransactionType.REVERSAL.
 *   - normalizedPayload gets "originalTxnRef" key for RV records
 *     (derived as the original sourceRef by convention — settlement engine uses it).
 *   - No change to record width (still 172 chars).
 */
public class NeftAdapter implements TransactionAdapter {
     ...
     * "NEFT260406001" → index 4-9 = "260406" (yyMMdd) → 2026-04-06
     */
    private LocalDate extractValueDateFromNeftRef(String neftRef) {
        if (neftRef.length() < 10) {
            throw new IllegalArgumentException(
                "NeftAdapter: NEFT_REF too short to extract date. Got: " + neftRef
            );
        }
    ...
    private String buildNormalizedPayload(String sourceRef, TransactionType txnType,
        ...
        return "{"
            ...
            + "\"originalTxnRef\":\"" + nullSafe(originalTxnRef) + "\","
