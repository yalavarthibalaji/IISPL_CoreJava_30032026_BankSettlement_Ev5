 ...
 *
 * Fintech partners (PhonePe, Razorpay, PayU etc.) send proprietary JSON via
 * REST webhooks with nested sender/receiver objects.
 *
 * RAW PAYLOAD FORMAT (Proprietary JSON — 20 fields including nested):
 * ────────────────────────────────────────────────────────────────────── {
 * "ft_ref" : "FT-2026-00112233", ← sourceRef "partner_id" :
 * "PHONEPE_PARTNER_01", "sender" : { "account" : "ACC-PH-123456", ←
 * debitAccount "name" : "Suresh Nair", "kyc_level" : "FULL" }, "receiver" : {
 * "account" : "ACC-PH-789012", ← creditAccount "name" : "Kavya Stores", "type"
 * : "MERCHANT" }, "txn_amount" : "12500.00", "txn_currency" : "INR",
 * "txn_category" : "P2M / P2P / REFUND / FEE", "initiated_at" :
 * "2026-04-02T13:00:00Z", "risk_score" : "LOW", "platform" : "ANDROID",
 * "wallet_id" : "WLT-9900112", "promo_code" : "SAVE10", "original_ref" :
 * "FT-2026-00112233", ← NEW — populated for REFUND only "reversal_reason":
 * "DUPLICATE_PAYMENT", ← NEW — populated for REFUND only "metadata" : {
 * "order_id":"ORD-77221", "store_id":"STR-445" } }
 * ──────────────────────────────────────────────────────────────────────
 *
 * txn_category → TransactionType mapping: P2M → CREDIT P2P → CREDIT REFUND →
 * REVERSAL (undo of a previous fintech payment) FEE → FEE
 *
 * For REFUND transactions: - original_ref must be present and non-empty -
 * reversal_reason must be present and non-empty - sender and receiver accounts
 * are swapped vs the original transaction
 *
 * CHANGE LOG (v4 — REVERSAL support): - original_ref and reversal_reason
 * top-level JSON fields added. - Both parsed and stored in normalizedPayload as
 * "originalTxnRef" and "reversalReason". - Validation: REFUND category requires
 * non-empty original_ref and reversal_reason. - fintech_transactions.json
 * updated with two REFUND entries.
 */
public class FintechAdapter implements TransactionAdapter {

	private static final DateTimeFormatter FINTECH_DATE_FORMAT = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss");
	...
	public IncomingTransaction adapt(String rawPayload) {
		...
		if (sourceRef == null || amountStr == null || currency == null || initiatedAt == null || senderAccount == null
				|| receiverAccount == null) {
			throw new IllegalArgumentException("FintechAdapter: Missing required fields. "
		...
		if (txnType == TransactionType.REVERSAL) {
			if (originalRef.isEmpty()) {
				throw new IllegalArgumentException(
						"FintechAdapter: REFUND transaction must have a non-empty 'original_ref'. " + "Got: "
								+ rawPayload);
			}
			if (reversalReason.isEmpty()) {
				throw new IllegalArgumentException(
						"FintechAdapter: REFUND transaction must have a non-empty 'reversal_reason'. " + "Got: "
								+ rawPayload);
			}
		}
		...
		String initiatedAtClean = initiatedAt.endsWith("Z") ? initiatedAt.substring(0, initiatedAt.length() - 1)
				: initiatedAt;
		...
		String normalizedPayload = buildNormalizedPayload(sourceRef, txnType, amount, currency, valueDate,
				senderAccount, receiverAccount, partnerId, senderName, senderKycLevel, receiverName, receiverType,
				nullSafe(txnCategory), riskScore, platform, walletId, promoCode, orderId, storeId, originalRef,
				reversalReason);

		// ---- Build IncomingTransaction ----
		IncomingTransaction txn = new IncomingTransaction(fintechSourceSystem, sourceRef, rawPayload, txnType, amount,
	 ...
	 *
	 * P2M → CREDIT (Person to Merchant payment) P2P → CREDIT (Person to Person
	 * payment) REFUND → REVERSAL (Refund = reversal of a previous payment) FEE →
	 * FEE (Platform fee charge) default→ CREDIT
	 */
	...
	private String buildNormalizedPayload(String sourceRef, TransactionType txnType, BigDecimal amount, String currency,
			LocalDate valueDate, String debitAcc, String creditAcc, String partnerId, String senderName,
			String senderKycLevel, String receiverName, String receiverType, String txnCategory, String riskScore,
			String platform, String walletId, String promoCode, String orderId, String storeId, String originalTxnRef,
			String reversalReason) {
		return "{" + "\"source\":\"FINTECH\"," + "\"sourceRef\":\"" + sourceRef + "\"," + "\"txnType\":\""
				+ txnType.name() + "\"," + "\"amount\":" + amount + "," + "\"currency\":\"" + currency + "\","
				+ "\"valueDate\":\"" + valueDate + "\"," + "\"debitAccount\":\"" + debitAcc + "\","
				+ "\"creditAccount\":\"" + creditAcc + "\"," + "\"partnerId\":\"" + nullSafe(partnerId) + "\","
				+ "\"senderName\":\"" + nullSafe(senderName) + "\"," + "\"senderKycLevel\":\""
				+ nullSafe(senderKycLevel) + "\"," + "\"receiverName\":\"" + nullSafe(receiverName) + "\","
				+ "\"receiverType\":\"" + nullSafe(receiverType) + "\"," + "\"txnCategory\":\"" + nullSafe(txnCategory)
				+ "\"," + "\"riskScore\":\"" + nullSafe(riskScore) + "\"," + "\"platform\":\"" + nullSafe(platform)
				+ "\"," + "\"walletId\":\"" + nullSafe(walletId) + "\"," + "\"promoCode\":\"" + nullSafe(promoCode)
				+ "\"," + "\"orderId\":\"" + nullSafe(orderId) + "\"," + "\"storeId\":\"" + nullSafe(storeId) + "\","
				+ "\"originalTxnRef\":\"" + originalTxnRef + "\"," + "\"reversalReason\":\"" + reversalReason + "\""
				+ "}";
	}
