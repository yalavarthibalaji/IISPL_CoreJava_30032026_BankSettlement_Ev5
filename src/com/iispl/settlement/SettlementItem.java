...
/**
 * SettlementItem — A wrapper that holds ONE transaction of any type along with
 * the channel it came from.
 *
 * WHY THIS CLASS EXISTS: The BlockingQueue in SettlementEngine needs to hold
 * different transaction types together: CreditTransaction, DebitTransaction,
 * InterBankTransaction, ReversalTransaction
 *
 * Java's BlockingQueue is typed — it can hold only ONE type. Solution: wrap all
 * transaction types inside this single class and put SettlementItem objects on
 * the queue.
 *
 * The consumer then checks getChannel() to decide which batch this item belongs
 * to, and checks getTxnType() to know which field to use (getCredit(),
 * getDebit(), getInterbank(), getReversal()).
 *
 * HOW TO USE: // Producer — wrapping a CreditTransaction for CBS channel:
 * SettlementItem item = new SettlementItem("CBS", credit, null, null, null,
 * incomingTxnId); queue.put(item);
 *
 * // Consumer — unwrapping: if (item.getChannel().equals("CBS") &&
 * item.getCredit() != null) { CreditTransaction txn = item.getCredit(); }
 *
 * CHANNEL VALUES (matches reference_number prefix): "CBS" — Core Banking System
 * "RTGS" — Real-Time Gross Settlement "NEFT" — National Electronic Funds
 * Transfer "UPI" — Unified Payments Interface "FT" — FinTech / Third-party API
 * "SHUTDOWN" — Sentinel signal to stop the consumer thread
 *
 * TXN_TYPE VALUES: "CREDIT" — CreditTransaction "DEBIT" — DebitTransaction
 * "INTERBANK" — InterBankTransaction "REVERSAL" — ReversalTransaction
 * "SHUTDOWN" — Sentinel
 *
...
public class SettlementItem {
	...
	// Which transaction type — one of these four will be non-null, rest will be
	// null
	...
	private SettlementItem(String channel, String txnType, CreditTransaction credit, DebitTransaction debit,
			InterBankTransaction interbank, ReversalTransaction reversal, Long incomingTxnId) {
	 ...
	 */
	public static SettlementItem ofCredit(String channel, CreditTransaction credit, Long incomingTxnId) {
	 ...
	 * @param incomingTxnId FK for settlement_record table
	 */
	public static SettlementItem ofDebit(String channel, DebitTransaction debit, Long incomingTxnId) {
	 ...
	 * @param incomingTxnId FK for settlement_record table
	 */
	public static SettlementItem ofInterbank(String channel, InterBankTransaction interbank, Long incomingTxnId) {
	 ...
	 * @param incomingTxnId FK for settlement_record table
	 */
	public static SettlementItem ofReversal(String channel, ReversalTransaction reversal, Long incomingTxnId) {
		return new SettlementItem(channel, "REVERSAL", null, null, null, reversal, incomingTxnId);
	}

	/**
	 * Creates a shutdown sentinel item. The consumer thread checks for this and
	 * stops its loop when it sees it.
	 */
	...
	public String toString() {
		String ref = "?";
		if (credit != null)
			ref = credit.getReferenceNumber();
		if (debit != null)
			ref = debit.getReferenceNumber();
		if (interbank != null)
			ref = interbank.getReferenceNumber();
		if (reversal != null)
			ref = reversal.getReferenceNumber();
		return "SettlementItem{channel='" + channel + "', txnType='" + txnType + "', ref='" + ref + "', incomingTxnId="
				+ incomingTxnId + "}";
	}
}