 ...
 *
 * WHY NOT A STATIC BLOCK ANYMORE?
 *   The original code used a static block to initialize HikariDataSource.
 *   If the DB connection fails inside a static block, Java marks the class
 *   as PERMANENTLY BROKEN for the rest of the JVM session. Every future
 *   access to that class throws NoClassDefFoundError — even in other threads.
 *
 *   This was causing the error:
 *     "NoClassDefFoundError: Could not initialize class ConnectionPool"
 *
...
public class ConnectionPool {
        ...
        synchronized (LOCK) {
            ...
            try {
                ...
                if (url == null || url.trim().isEmpty()) {
                    throw new RuntimeException(
                        "CONNECTION_STRING is missing or empty in db.properties"
                    );
                }
                ...
                throw new RuntimeException(
                    "[ConnectionPool] Failed to initialize: " + e.getMessage() +
     ...
     * it does NOT actually close the physical connection.
     *
     ...
     * Call this only after ALL threads have finished their DB work.
     * After this is called, getConnection() will fail.
     */
